...
assert.ok(localTimeFix.includes('isTerminalStatus') && /hủy phiếu/i.test(localTimeFix), 'Thiếu khóa hủy trực tiếp phiếu sửa chữa đã kết thúc trên giao diện.');
assert.ok(scopeGuard.includes("app.use('/api'"), 'Guard API phải dùng middleware /api hiện hữu.');
assert.ok(scopeGuard.includes("req.method === 'DELETE'"), 'Guard API phải kiểm tra yêu cầu DELETE phiếu sửa chữa.');
assert.ok(scopeGuard.includes("scopedPath.match(/^\\/repairs\\/(\\d+)$/)"), 'Guard API phải nhận đúng đường dẫn /repairs/:id khi mount dưới /api.');
