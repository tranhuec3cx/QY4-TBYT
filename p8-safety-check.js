...
assert.ok(scopeGuard.includes("app.use('/api/repairs/:id'"), 'Thiếu guard API chống hủy phiếu sửa chữa đã kết thúc.');
assert.ok(scopeGuard.includes("req.method !== 'DELETE'"), 'Guard API phải chỉ can thiệp yêu cầu DELETE phiếu sửa chữa.');
